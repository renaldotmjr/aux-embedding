...
!      ALLOCATE( EMB_COEF(NSHL+1, 1))
      ALLOCATE( H(NSHL+1, NSHL+1) )
      ! Allocating the eigenvalues and eigenvectors vectors
      ALLOCATE( EIGVAL(NSHL+1), EIGVEC(NSHL+1, NSHL+1) )
      ! Allocating transformed vectors
      ALLOCATE( B_trf(NSHL+1, 1), EIGVEC_T(NSHL+1, NSHL+1) )

      ! Allocating EMB_COEF
      IF (ALLOCATED(EMB_COEF)) THEN
      ELSE
          ALLOCATE( EMB_COEF(NSHL+1, 1) )
      ENDIF
      ALLOCATE( X_trf(NSHL+1, 1) )


      ALLOCATE( XXX(NSHL+1), IQ(NSHL+1) )
      ...
      ! Testing if H is a symetric matrix
      SYM = 0
      DO J=1, N
          DO K=1, N

              IF ( DABS( H(J,K)-H(K,J) ) > 1.0D-12 ) THEN

                  WRITE (6, *) J, K, H(J,K), H(K,J)
                  SYM = SYM+1
      ...
      IF (SYM .NE. 0) THEN
          WRITE (6, *) 'Non-symmetric overlap matrix!'
          WRITE (6, *) 'Matrix elements with same value=', SYM
      ELSE
          WRITE (6, *) 'Overlap matrix is symmetric! Solving LE!'
      ENDIF
...
!     **************** END-OF DEBUG print! ***************
      ...
      EMB_COEF = 0.0D0
      EMB_COEF = MATMUL(EIGVEC, X_trf)          ! X = U * X_transf
...
!     ******************* DEBUG print! *******************
      IF (DEBUG_INV) THEN
         ...
         WRITE(6,*)
         DO I=1, N
            WRITE(6,'(I5, F16.10, F15.10, I10)') I, EMB_COEF(I,1), XXX(I), IQ(I)
         ENDDO
         WRITE(6,*)
